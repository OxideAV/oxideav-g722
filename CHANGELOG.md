...
## [0.0.8](https://github.com/OxideAV/oxideav-g722/compare/v0.0.7...v0.0.8) - 2026-08-13

### Other

- held-out validation of the fitted pitch preference on test20
- fitted smaller-pitch preference for the Appendix IV LTP search
- continuation-path unit tests + provenance docs for the staged PLC sources
- README + CHANGELOG — Appendix IV PLC fixed-point rebuild status
- PLC signal path + classifier + IV.6.1.3 continuation rebuilt fixed-point; vector-calibrated
- fixed-point PLC analysis machinery (plc_analysis) on the staged tables + operators
- Appendix IV groundwork — staged STL basic-operator set + Table IV.5 numeric tables
- crate docs — surface the Appendix IV PLC entry point and its provenance
- spec-anchored PLC unit tests — muting trajectories, remove-DC floor, voiced continuation
- plc_frames fuzz target — PLC totality + mute-floor invariants
- README — T-series conformance gap closed, Appendix IV PLC status
- Appendix IV packet-loss concealment (PlcDecoder) + staged-vector characterisation
- Appendix II T-series conformance harness — all 11 legs bit-exact
