//! Clause 2.5.1 / Figure 11/G.722 — input anti-aliasing filter mask.
//!
//! Clause 2.5.1 (p. 11) of the staged ITU-T G.722 (11/88) Recommendation
//! requires the transmit audio part's input anti-aliasing filter to
//! satisfy the attenuation/frequency-response limits drawn in
//! Figure 11/G.722 (p. 12, "Attenuation/frequency response of the input
//! antialiasing filter"). The clause is evaluated at test point A of
//! Figure 2/G.722 (p. 2), driving the input of the transmit audio part
//! that precedes the SB-ADPCM encoder's 14-bit / 16 kHz uniform-PCM
//! sampling interface (clause 1.4.1 p. 4). The clause text quotes a
//! nominal reference frequency of 1000 Hz and an in-band test level of
//! −10 dBm0; out-of-band measurements account for aliasing due to the
//! 16 kHz sampling.
//!
//! Figure 11/G.722 prints the mask as a pair of piecewise-constant
...
//! | 9 kHz and above     | ≥ +50                                                                               | (no upper)      |
//!
//! As with [`super::reconstructing_filter`], the spec follows the
//! attenuation-positive sign convention: a measurement of `+0.3` dB
//! attenuation lies between the `−0.5` lower bound and the `+0.5` upper
//! bound and therefore meets the mask in the 100 Hz – 6.4 kHz in-band
//! region.
//!
//! ## Difference from Figure 12/G.722 (output reconstructing filter)
//!
//! Figures 11 and 12 share the in-band ripple corridor and stopband
//! entry exactly. They differ in the stopband floor:
//!
//! * Figure 11 (input anti-aliasing, [`Self`]): anchors at 25 dB / 8 kHz
//!   and 50 dB / 9 kHz; the 50 dB ceiling extends flat beyond 9 kHz to
//!   the band edge. There is **no** 14 kHz / 70 dB anchor (the
//!   anti-aliasing filter only needs to suppress content that would
//!   alias around the 8 kHz Nyquist, so the 70 dB / 14 kHz shoulder
//!   that Figure 12 spends on D/A x/sin x correction is not required).
//! * Figure 12 (output reconstructing filter,
//!   [`super::reconstructing_filter`]): anchors at 25 dB / 8 kHz, 50 dB
//!   / 9 kHz, **and** 70 dB / 14 kHz; the 50 dB ceiling between 9 kHz
//!   and 14 kHz is a log-linear ramp toward 70 dB before going flat at
//!   the top.
//!
//! The two masks therefore share their breakpoint list up to 9 kHz; the
//! constants below intentionally mirror those of
//! [`super::reconstructing_filter`] to make the parallel obvious and to
//! let downstream tooling treat the two filters with the same
//! `MaskBand` taxonomy.
//!
//! ## Provenance
//!
//! Every breakpoint and dB value below is transcribed from the printed
//! Figure 11/G.722 of `docs/audio/g722/T-REC-G.722-198811-S.pdf` (page
...
// -----------------------------------------------------------------------
// Mask breakpoints (Figure 11/G.722 page 12)
// -----------------------------------------------------------------------

/// In-band passband low edge of Figure 11/G.722 (p. 12). The mask's
...
/// Figure 11/G.722 (p. 12). Above this the upper bound relaxes to
/// +1.5 dB up to [`PASSBAND_RELAXED_HIGH_HZ`].
pub const PASSBAND_TIGHT_HIGH_HZ: u32 = 6_400;

/// Upper edge of the relaxed in-band ripple region (Figure 11/G.722
/// p. 12). Beyond this the mask transitions into the stopband.
pub const PASSBAND_RELAXED_HIGH_HZ: u32 = 7_000;

/// Out-of-band stopband entry (Figure 11/G.722 p. 12). At 8 kHz the
/// anti-aliasing filter must already provide ≥ 25 dB of attenuation.
pub const STOPBAND_ENTRY_HZ: u32 = 8_000;

/// First (and only) stopband shoulder (Figure 11/G.722 p. 12). At 9 kHz
/// and above the mask requires ≥ 50 dB of attenuation. Unlike the
/// receive-side Figure 12/G.722 mask, no further anchor is printed; the
/// 50 dB floor extends flat to the band edge.
pub const STOPBAND_SHOULDER_HZ: u32 = 9_000;

/// Lower bound of the tight in-band ripple band (100 Hz – 6.4 kHz) in dB
/// (Figure 11/G.722 p. 12). Sign convention: attenuation positive, so
...
/// Minimum attenuation at [`STOPBAND_ENTRY_HZ`] (Figure 11/G.722 p. 12).
pub const STOPBAND_ENTRY_MIN_ATTEN_DB: f64 = 25.0;

/// Minimum attenuation at [`STOPBAND_SHOULDER_HZ`] and above
/// (Figure 11/G.722 p. 12). The figure does not print a higher anchor;
/// the 50 dB attenuation floor extends flat beyond 9 kHz.
...
/// against the Figure 11/G.722 mask.
///
/// The variants follow the same partitioning as
/// [`super::reconstructing_filter::MaskBand`] except that Figure 11/G.722
/// has no analogue of the 9 kHz – 14 kHz log-linear ramp: above
/// [`STOPBAND_SHOULDER_HZ`] the floor is a flat 50 dB ceiling.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum MaskBand {
    /// Frequency is below the 50 Hz mask anchor — outside the
    /// anti-aliasing filter's normative coverage.
    BelowMask,
    /// Frequency sits inside the 50 Hz – [`PASSBAND_LOW_HZ`] transition
    /// strip. No tight numeric bound is printed in Figure 11/G.722; the
    ...
    /// printed in Figure 11/G.722 (−0.5 dB); the upper bound rolls up
    /// to the [`STOPBAND_ENTRY_MIN_ATTEN_DB`] floor.
    HighTransition,
    /// Frequency is in the stopband ramp
    /// [`STOPBAND_ENTRY_HZ`] – [`STOPBAND_SHOULDER_HZ`]. Only a lower
    /// bound on attenuation applies; the floor rises log-linearly from
    /// 25 dB at 8 kHz to 50 dB at 9 kHz.
    StopbandRamp,
    /// Frequency is in the flat-floor stopband
    /// (≥ [`STOPBAND_SHOULDER_HZ`]). Only a lower bound on attenuation
    /// applies; the floor is the flat 50 dB ceiling.
    StopbandFlat,
}

/// Evaluate a measured `(frequency_hz, attenuation_db)` pair against the
/// Figure 11/G.722 mask.
///
...
/// | `StopbandRamp`    | `atten ≥ floor(frequency)` (see [`stopband_floor_db`]) |
/// | `StopbandFlat`    | `atten ≥ 50 dB`                                        |
pub fn evaluate(frequency_hz: f64, attenuation_db: f64) -> (MaskBand, bool) {
    ...
    let ok = match band {
        ...
        MaskBand::StopbandRamp => attenuation_db >= stopband_floor_db(frequency_hz),
        MaskBand::StopbandFlat => attenuation_db >= STOPBAND_SHOULDER_MIN_ATTEN_DB,
    };
    (band, ok)
}
...
pub fn classify(frequency_hz: f64) -> MaskBand {
    ...
    if frequency_hz < STOPBAND_SHOULDER_HZ as f64 {
        return MaskBand::StopbandRamp;
    }
    MaskBand::StopbandFlat
}

/// Minimum attenuation (in dB) the input anti-aliasing filter must
/// provide at `frequency_hz` once the frequency lies in the stopband
/// (≥ [`STOPBAND_ENTRY_HZ`]). Figure 11/G.722 (p. 12) prints two
/// anchor points: 25 dB at 8 kHz and 50 dB at 9 kHz; the 8–9 kHz
/// segment is piecewise-linear on a log-frequency axis (the printed
/// mask shows a straight diagonal segment on the log scale), and the
/// 50 dB ceiling extends flat beyond 9 kHz.
///
...
pub fn stopband_floor_db(frequency_hz: f64) -> f64 {
    ...
    // ≥ 9 kHz — Figure 11/G.722's flat 50 dB ceiling.
    STOPBAND_SHOULDER_MIN_ATTEN_DB
}
...
const _: () = {
    ...
    // SUBBAND_SAMPLE_CLOCK_HZ, Figure 11/G.722's vertical line at 8 kHz
    // marks exactly that point — the Nyquist edge of the 16 kHz
    // sampling that follows the anti-aliasing filter).
...
mod tests {
    use super::*;
    use crate::transmission::reconstructing_filter;

    #[test]
    fn breakpoints_match_figure_11() {
        // The printed Figure 11/G.722 (p. 12) shows the labelled
        // frequencies 0.050 / 0.100 / 1 / 6.4 / 7 / 8 / 9 kHz on the
        // log frequency axis. The mask's anchor points must match.
    ...
    #[test]
    fn ripple_bounds_match_figure_11() {
        // Figure 11/G.722 (p. 12) prints −0.5 / +0.5 / +1.5 dB on the
        // attenuation axis for the in-band rectangles. The stopband
        // labels are 25 dB and 50 dB.
    ...
    #[test]
    fn classify_stopband_ramp() {
        // 8 kHz to (just under) 9 kHz: log-linear ramp from 25 dB to
        // 50 dB.
        assert_eq!(classify(8_000.0), MaskBand::StopbandRamp);
        assert_eq!(classify(8_500.0), MaskBand::StopbandRamp);
        assert_eq!(classify(8_999.999), MaskBand::StopbandRamp);
    }

    #[test]
    fn classify_stopband_flat() {
        // 9 kHz and above: flat 50 dB ceiling.
        assert_eq!(classify(9_000.0), MaskBand::StopbandFlat);
        assert_eq!(classify(14_000.0), MaskBand::StopbandFlat);
        assert_eq!(classify(50_000.0), MaskBand::StopbandFlat);
    }
    ...
    fn evaluate_stopband_anchor_25_db_at_8khz() {
        ...
        // 11/G.722 p. 12) must just meet the mask.
        let (band, ok) = evaluate(8_000.0, 25.0);
        assert_eq!(band, MaskBand::StopbandRamp);
        assert!(ok);
    ...
    fn evaluate_stopband_anchor_50_db_at_9khz() {
        let (band, ok) = evaluate(9_000.0, 50.0);
        assert_eq!(band, MaskBand::StopbandFlat);
        assert!(ok);
        let (band, ok) = evaluate(9_000.0, 49.0);
        assert_eq!(band, MaskBand::StopbandFlat);
        assert!(!ok);
    }

    #[test]
    fn evaluate_far_stopband_meets_flat_50_db() {
        // 14 kHz, 50 dB: meets the flat 50 dB ceiling of Figure 11.
        // (Figure 12 demands 70 dB at the same frequency; Figure 11
        // does not. This is the principal axis on which the two masks
        // diverge.)
        let (band, ok) = evaluate(14_000.0, 50.0);
        assert_eq!(band, MaskBand::StopbandFlat);
        assert!(ok);
        // 49 dB at 14 kHz fails Figure 11 (and would also fail the
        // tighter Figure 12 anchor at that frequency).
        let (_, ok) = evaluate(14_000.0, 49.0);
        assert!(!ok);
    }
    ...
    fn stopband_floor_anchor_values() {
        // Exact anchor points from Figure 11/G.722 (p. 12).
    ...
    fn stopband_floor_is_monotone_non_decreasing_with_frequency() {
        // Figure 11/G.722's printed stopband is a rising step/slope
        // that flattens out at 9 kHz. Sample 8 kHz – 20 kHz on a
        // 100 Hz step grid; the floor must never drop.
    ...
    #[test]
    fn stopband_floor_above_9khz_is_flat_50db() {
        // The mask shows a flat 50 dB ceiling extending past 9 kHz to
        // the band edge (no further anchor printed).
        assert_eq!(stopband_floor_db(10_000.0), 50.0);
        assert_eq!(stopband_floor_db(14_000.0), 50.0);
        assert_eq!(stopband_floor_db(20_000.0), 50.0);
        assert_eq!(stopband_floor_db(40_000.0), 50.0);
    }
    ...
    fn evaluate_low_transition_always_passes() {
        // Figure 11/G.722 doesn't pin a numeric upper bound inside the
    ...
    #[test]
    fn shares_in_band_corridor_with_figure_12() {
        // Per the docs Figures 11 and 12 share their in-band ripple
        // corridor exactly: −0.5 / +0.5 dB tight, +1.5 dB relaxed, and
        // the same 100 Hz / 6.4 kHz / 7 kHz breakpoints. Pin that
        // explicitly so a future edit to either mask flags the drift.
        assert_eq!(
            IN_BAND_LOWER_BOUND_DB,
            reconstructing_filter::IN_BAND_LOWER_BOUND_DB
        );
        assert_eq!(
            IN_BAND_TIGHT_UPPER_BOUND_DB,
            reconstructing_filter::IN_BAND_TIGHT_UPPER_BOUND_DB
        );
        assert_eq!(
            IN_BAND_RELAXED_UPPER_BOUND_DB,
            reconstructing_filter::IN_BAND_RELAXED_UPPER_BOUND_DB
        );
        ...
        assert_eq!(STOPBAND_ENTRY_HZ, reconstructing_filter::STOPBAND_ENTRY_HZ);
        assert_eq!(
            STOPBAND_ENTRY_MIN_ATTEN_DB,
            reconstructing_filter::STOPBAND_ENTRY_MIN_ATTEN_DB
        );
        // And the 9 kHz / 50 dB shoulder is identical too.
        assert_eq!(
            STOPBAND_SHOULDER_HZ,
            reconstructing_filter::STOPBAND_SHOULDER_HZ
        );
        assert_eq!(
            STOPBAND_SHOULDER_MIN_ATTEN_DB,
            reconstructing_filter::STOPBAND_SHOULDER_MIN_ATTEN_DB
        );
    }

    #[test]
    fn diverges_from_figure_12_above_9khz() {
        // Figure 11 has no 14 kHz / 70 dB anchor — its floor stays at
        // 50 dB. Figure 12 ramps from 50 dB (9 kHz) to 70 dB (14 kHz)
        // and then flattens at 70 dB. At 14 kHz the two floors must
        // therefore differ by exactly 20 dB.
        let here = stopband_floor_db(14_000.0);
        let there = reconstructing_filter::stopband_floor_db(14_000.0);
        assert!((here - 50.0).abs() < 1e-9);
        assert!((there - 70.0).abs() < 1e-9);
        assert!((there - here - 20.0).abs() < 1e-9);
    }
    ...
    fn evaluate_matches_classify_for_passing_measurement_at_each_anchor() {
        ...
        let pairs: [(f64, f64, MaskBand); 7] = [
            (1_000.0, 0.0, MaskBand::InBandTight),
            (6_400.0, 0.5, MaskBand::InBandTight),
            (6_400.0, -0.5, MaskBand::InBandTight),
            (7_000.0, 1.5, MaskBand::InBandRelaxed),
            (7_000.0, -0.5, MaskBand::InBandRelaxed),
            (8_000.0, 25.0, MaskBand::StopbandRamp),
            (9_000.0, 50.0, MaskBand::StopbandFlat),
        ];
